 ...
 * @Author: yang
 * @CreateTime: 2019-04-29 15:43
 * @Description: 回访园所打卡数据模板
 */
public class ReturnVisitExportExcel {

    public static void export(String provinceName, List<Map> resultMap, ServletOutputStream out, String startDate,String endDate) throws Exception{


        //标题
        String[] title = {"序号", "省","市","区","幼儿园", "班级名称", "心情", "平均心情", "饮水", "平均饮水", "饮食", "平均饮食", "睡眠", "平均睡眠",
        ...
        //插入第一行数据 题目
        String mytitle=provinceName+"幼儿园"+startDate+"-"+endDate+"打卡数据";
        cell = firstRow.createCell(0);
        cell.setCellValue(mytitle);
        cell.setCellStyle(titleStyle);
        CellRangeAddress region = new CellRangeAddress(0, 0, 0, 25);//合并单元格
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (resultMap.get(i).get("province_name") != null) {
                cell2.setCellValue(resultMap.get(i).get("province_name").toString());
            } else {
                cell2.setCellValue("");
            }
            sheet.setColumnWidth(1, 20 * 256);//设置省一列的宽高
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(2);
            if (resultMap.get(i).get("city_name") != null) {
                cell2.setCellValue(resultMap.get(i).get("city_name").toString());
            } else {
                cell2.setCellValue("");
            }
            sheet.setColumnWidth(2, 10* 256);//设置市一列的宽高
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(3);
            if (resultMap.get(i).get("area_name") != null) {
                cell2.setCellValue(resultMap.get(i).get("area_name").toString());
            } else {
                cell2.setCellValue("");
            }
            sheet.setColumnWidth(3, 20 * 256);//设置区一列的宽高
            cell2.setCellStyle(titleStyle3);
            ...
            sheet.setColumnWidth(4, 20 * 256);//设置幼儿园一列的宽高
            cell2.setCellStyle(titleStyle3);
            ...
            if (resultMap.get(i).get("leave_count") != null) {
                cell2.setCellValue(resultMap.get(i).get("leave_count").toString());
            } else {
                cell2.setCellValue("");
            }
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(21);
            if (resultMap.get(i).get("leave_avg") != null) {
                cell2.setCellValue(resultMap.get(i).get("leave_avg").toString());
            } else {
                cell2.setCellValue("");
            }
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(22);
            if (resultMap.get(i).get("people_count") != null) {
                cell2.setCellValue(resultMap.get(i).get("people_count").toString());
            } else {
                cell2.setCellValue("");
            }
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(23);
            if (resultMap.get(i).get("count") != null) {
                cell2.setCellValue(resultMap.get(i).get("count").toString());
            } else {
                cell2.setCellValue("");
            }
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(24);
            if (resultMap.get(i).get("count_avg") != null) {
                cell2.setCellValue(resultMap.get(i).get("count_avg").toString());
            } else {
                cell2.setCellValue("");
            }
            cell2.setCellStyle(titleStyle3);
            cell2 = nextrow.createCell(25);
            if (resultMap.get(i).get("status") != null) {
                cell2.setCellValue(resultMap.get(i).get("status").toString());
            } else {
                cell2.setCellValue("");
            }
            cell2.setCellStyle(titleStyle3);
